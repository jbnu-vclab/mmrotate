...
class evaluate_from_txt():
        ...
        for root, dirs, files in os.walk(txt_dir_path):
            for filename in files:
                if filename.endswith('.txt'):
                    ...
                    result[filename.replace('.txt','')] = self.__anntxt2dict(file_path)
        return result
    ...
    def visualize(self, gt_imgs_path: str, write_path: str, cls: str = 'text'):
        ...
        for diagram in tqdm(dt_dict.keys(), f"Visualizing '{cls}' Class"):
            diagram = diagram.replace('.txt', '')
            gt_img_path = os.path.join(gt_imgs_path, f"{diagram}.png")
            ...
            for dt_item in dt_items:
                ...
                up_seg_mid_x = (x1 + x2) / 2
                up_seg_mid_y = (y1 + y2) / 2

                up_vec = [up_seg_mid_x - c_x, up_seg_mid_y - c_y]
                up_vec = up_vec / np.linalg.norm(up_vec)

                up_point = [int(up_seg_mid_x + up_vec[0] * 15), int(up_seg_mid_y + up_vec[1] * 15)]

                cv2.line(vis_img, [int(up_seg_mid_x), int(up_seg_mid_y)], up_point, (0, 0, 255), 2)

                dt_cls = dt_item[1]
                if dt_cls == cls or cls == 'all':
                    for num in range(4):
                        cv2.line(vis_img, dt_points[(num + 0) % 4], dt_points[(num + 1) % 4], (0, 0, 255), 1)
...
root_dir = '/data/mmrotate_test_results/dota_adaptive_lambda'
classtxt_dir_path = root_dir
gt_imgs_path = '/data/dotav1/test/images'
gt_anntxts_path = '/data/split_ss_dota/test/annfiles'
dt_anntxts_path = root_dir + '/dt_anns'
symbol_txt_path = 'D:\\Data\\SymbolClass_Class.txt'
dump_path = '/data/mmrotate_test_results/'
visualize_path = root_dir + '/vis'
